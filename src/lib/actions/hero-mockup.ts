...
export async function generateHeroMockup(
	...
	// Idempotency: skip if already in-flight or done. Callers can poll via
	// `checkHeroMockupStatus` instead of generating a second task.
	if (
	...
	try {
		...
		const { error: updateError } = await adminDb
			...
			.update({
				...
				// leak stale URLs through `checkHeroMockupStatus`.
		...
		// Best-effort persist of the failure so admin + retry UX sees it.
		await adminDb
			.from('orders')
			.update({
...
export async function checkHeroMockupStatus(
	...
	// Short-circuit terminal states so we don't waste a Kie poll.
	...
	try {
		...
		if (taskStatus.state === 'success' && taskStatus.resultUrls?.length) {
			...
			const previewPath =
				`${ctx.data.storagePrefix}/hero-mockup-previews/${order.id}.png`
